...
class Platform:
    ...
    def get_available_names():
        """
        Returns a list of potentially available platform names (e.g., ['cpu', 'cuda'])
        based on quick checks, without performing full device enumeration or usability tests.
        This is intended for populating argparse choices to ensure fast startup.
        """
        names = ["cpu"]
        # This check is quick and does not trigger the slow CUDA context initialization.
        if _numba_imported and cuda.is_available():
            names.append("cuda")
        return names

    def __init__(self, platform_name: str = "cpu", debug: bool = False) -> None:
        ...
        try:
            if platform_name.lower() == "cuda" and _numba_imported:
                if self.debug:
                    print(
    ...
    def _test_cuda(self):
        ...
        try:
            ...
            result_cu_dev = cuda.to_device(result_cu)
