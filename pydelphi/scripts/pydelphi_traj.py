...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        prog="pydelphi-trajectory.py",
        description="Run pydelphi for a trajectory (PBE calculation).",
    )
    ...
    parser.add_argument(
        ...
        default="notice",
        help="Verbosity level for outputs (default: notice).",
    )
    parser.add_argument(
...
def main():
    ...
    from pydelphi.app.delphi_trajectory import DelphiTrajApp

    log_file = args.outfile.replace(".csv", "_traj.log")

    try:
        app = DelphiTrajApp(args.param_file, platform, user_inputs=None)

        with open(log_file, "w") as log_fp:
            energies = app.run(args.outfile, log_fp, args.label, args.overwrite)

    except FileNotFoundError as exc:
        missing = getattr(exc, "filename", None)

        if missing:
            print(f"❌ Error: Required file not found: {missing}")
        else:
